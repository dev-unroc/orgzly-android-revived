...
class AttachmentSaveFiles(
    ...
    override fun run(dataRepository: DataRepository): UseCaseResult {
        ...
        return try {
            ...
            val canWriteToSyncDir = if (parentDir != null) {
                try {
                    val testAttachDir = File(parentDir, ".attach")
    ...
    private fun getBookFileForAttachments(dataRepository: DataRepository, bookView: BookView): File {
        ...
        // This is needed for creating .attach directories relative to the org file
                ...
                when {
                    ...
                    repo.url.startsWith("content://com.android.externalstorage.documents/tree/primary") -> {
                        ...
                        if (parentDir != null) {
                            ...
                            // Test if we can write to this directory by trying to create a .attach subdirectory
                            val attachDir = File(parentDir, ".attach")
                            val canWrite = try {
                                if (!attachDir.exists()) {
                                    attachDir.mkdirs()
                                } else {
